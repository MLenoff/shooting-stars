...
export default async function TrainingPackagesPage() {
  ...
  const packages = [
    ...
    programs.find(p => p.id === 'training-20pack'),
  ].filter(Boolean);

  const perSession: Record<string, string> = {
    'individual-training': '$72.80 / session',
    'training-10pack': '$41.60 / session',
    'training-20pack': '$36.40 / session',
  };

  const highlight: Record<string, string | undefined> = {
    'individual-training': undefined,
    'training-10pack': 'Most Popular',
    'training-20pack': 'Best Value',
  };
